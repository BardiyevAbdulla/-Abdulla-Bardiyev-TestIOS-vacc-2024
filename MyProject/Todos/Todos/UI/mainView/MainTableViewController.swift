...
class MainTableViewController: BaseViewController, UISearchResultsUpdating {
    ...
    override func loadView() {
        ...
        vm.pagination.fetchData()
        vm.pagination.$items.receive(on: DispatchQueue.main)
...
extension MainTableViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if indexPath.row+1  == vm.pagination.items?.count ?? 1 {
            vm.pagination.update()
        }
    }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return vm.pagination.items?.count ?? 0
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var cell: UITableViewCell!
        if let item = vm.pagination.items?[indexPath.row] {
            cell = TodoCell(model: item, highlightedText: searchController.searchBar.text)
        } else {
            cell = UITableViewCell()
            cell.textLabel?.text = vm.pagination.items?[indexPath.row].title
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        guard let item = vm.pagination.items?[indexPath.row] else { return }
        vm.gotoDetail(item)
        // select
    }
}
